...
# Hàm xử lý file Word (.docx)
def process_word(file_path):
    """Xử lý file Word (DOCX) để lấy nội dung."""
    try:
        ...
        print(f"⚠️ Lỗi khi xử lý file Word {file_path}: {e}")
        return ""
        ...
        for file_name in tqdm(files, desc=f"📂 Đang xử lý ({folder_name})"):
            ...
            try:
                if file_name.endswith(".txt"):
                    ...
                    content = process_word(file_path)  # Gọi hàm xử lý file Word
